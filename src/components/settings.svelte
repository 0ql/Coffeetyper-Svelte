			...
			<div class="grid grid-cols-4 auto-rows-auto gap-3 p-3">
				...
				>
					{#if fonts && fontsFocus}
						...
						{/each}
					{:else}
						<option value={$settings.cosmetics.family}>{$settings.cosmetics.family}</option>
					{/if}
				</Select>
				...
				<Select class="col-span-4 h-10 w-full" bind:value={$settings.gen.filters.casing}>
					<option value="default">Default</option>
					<option value="lowercase">Lowercase</option>
					<option value="uppercase">Uppercase</option>
					<option value="random">Random</option>
					<option value="wordBeginning">Uppercase first letter of every Word</option>
				</Select>
