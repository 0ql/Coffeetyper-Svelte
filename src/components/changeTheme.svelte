...
{#if $settings.cosmetics.theme.opened === true}
	...
	>
		<div
			class="pr-5 grid grid-cols-2"
			transition:fly={{ x: 100, duration: 400 }}
		>
			{#each list as th}
				<div
					on:click={() => changeTheme(th.name)}
					class="m-2 rounded-lg p-2 cursor-pointer duration-150 transform hover:translate-x-[-0.5rem] {th.name ===
					$settings.cosmetics.theme.name
						? 'border-3'
						: ''} flex items-center"
					style="background: {th.bgColor}; color: {th.textColor}; border-color: {th.textColor}"
				>
					{th.name.replace(/_/g, " ")}
				</div>
			{/each}
